    ...
    const pushVLine = (t: number, major: boolean) => {
        ...
        lines.push(
            <line key={`gv-${t}`}
                  ...
                  stroke={major ? colors.grid.major : colors.grid.minor}
        ...
        if (major) {
            labels.push(
                <text key={`glx-${t}`} x={x} y={MARGIN - GRID_LABEL_OFFSET}
                      textAnchor="middle" fontSize={GRID_LABEL_FONT_SIZE} fill={colors.grid.label}>
                    {t.toFixed(2)}
                </text>
            )
        }
        ...
        if (major) {
            labels.push(
                <text key={`gly-${t}`} x={MARGIN - GRID_LABEL_OFFSET} y={y}
                      ...
                      fontSize={GRID_LABEL_FONT_SIZE} fill={colors.grid.label}>
                    {t.toFixed(2)}
                </text>
            )
        }
